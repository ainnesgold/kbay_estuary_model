#Sensitivity test on H_mort, P_mort

library(tidyverse)

H_mort <- seq(0, 0.1, by = 0.02)
P_mort <- seq(0, 0.1, by = 0.02)
...
parameter_grid <- expand.grid(H_mort = H_mort,
                              P_mort = P_mort)
number_patches = 1
#outputs
...
for (iter in 1:n) {
  ...
  for(t in 2:timesteps){
    ...
    N[t] = N[t-1] + ((-prod[t-1] + phmort[t-1] + y*grazing[t-1] + (parameter_grid[['H_mort']][[iter]] * H[t-1]) + 
                        (parameter_grid[['P_mort']][[iter]] * P[t-1])) *dt)
    
    H[t] = H[t-1] + ((r_H * H[t-1]) * (1 - H[t-1] / K_H) + ((1-y) * grazing[t-1]) - (parameter_grid[['H_mort']][[iter]]*H[t-1]) -
                       (c * H[t-1] * P[t-1]) / (d + H[t-1])
    ) * dt
    
    P[t] = P[t-1] + (r_P * P[t-1] * (1 - P[t-1] / K_P) - (parameter_grid[['P_mort']][[iter]]*P[t-1]) +
...
ggplot(outcome, aes(x=H_mort, y=outcome_N, col = as.factor(P_mort))) +
  geom_point() +
  geom_line()

ggplot(outcome, aes(x=H_mort, y=outcome_PH, col = as.factor(P_mort))) +
  geom_point() +
  geom_line()

ggplot(outcome, aes(x=H_mort, y=outcome_H, col = as.factor(P_mort))) +
  geom_point() +
  geom_line()

ggplot(outcome, aes(x=H_mort, y=outcome_P, col = as.factor(P_mort))) +
  geom_point() +
  geom_line()
